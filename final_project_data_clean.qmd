...
```{python}
# remove na (if column has more than 20% values missing then drop)
...
```{python}
# remove na (if column has more than 20% values missing then drop)
acled_CA = acled_CA.loc[:, acled_CA.isnull().mean() <= 0.2]

# examine remaining
acled_col = acled_CA.columns.to_list()
acled_col = pd.DataFrame(acled_col)
```
